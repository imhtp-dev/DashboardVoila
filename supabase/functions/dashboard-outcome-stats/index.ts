/**
 * Dashboard Outcome Stats Edge Function
 * Returns motivazione and esito_chiamata stats for pie charts
...
serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc('get_outcome_stats', {
      p_region: region,
    ...
    console.error('❌ Error loading outcome stats:', error);
