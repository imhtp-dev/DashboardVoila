 ...
 * Now uses Supabase Edge Functions instead of info_agent FastAPI
 */
...
export interface User {
  ...
  email: string;
  name: string;
  nome?: string;  // Optional for backwards compatibility
  cognome?: string;  // Optional for backwards compatibility
  role: string;
  ruolo?: string;  // Optional for backwards compatibility
  region: string;
  is_active: boolean;
  created_at: string;
  updated_at: string;
...
function getAuthHeaders(): HeadersInit {
  ...
  if (token) {
    // Use Authorization: Bearer with custom token (matches info_agent approach)
    headers['Authorization'] = `Bearer ${token}`;
  } else {
    // For unauthenticated requests, use anon key
...
export const usersApi = {
  ...
  async toggleStatus(userId: number): Promise<ToggleStatusResponse> {
    const response = await fetch(`${API_BASE_URL}/users-toggle-status?user_id=${userId}`, {
      method: 'POST',
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  ...
  async resendCredentials(userId: number): Promise<ResendCredentialsResponse> {
    const response = await fetch(`${API_BASE_URL}/users-resend-credentials`, {
      method: 'POST',
      headers: getAuthHeaders(),
  ...
  async delete(userId: number): Promise<DeleteResponse> {
    const response = await fetch(`${API_BASE_URL}/users-delete?user_id=${userId}`, {
      method: 'POST',
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
...
export const qaApi = {
  async listByRegion(region: string): Promise<QAItem[]> {
    const response = await fetch(`${API_BASE_URL}/qa-list-by-region/${encodeURIComponent(region)}`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  async create(data: { question: string; answer: string; region: string }): Promise<QACreateResponse> {
    const response = await fetch(`${API_BASE_URL}/qa-create`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify(data),
    });
  ...
  async update(qaId: number, data: { question: string; answer: string }): Promise<QAUpdateResponse> {
    const response = await fetch(`${API_BASE_URL}/qa-update`, {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ qa_id: qaId, ...data }),
    });
    return handleResponse(response);
  },

  async delete(qaId: number): Promise<QADeleteResponse> {
    const response = await fetch(`${API_BASE_URL}/qa-delete?qa_id=${qaId}`, {
      method: 'POST',
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
...
export const dashboardApi = {
  ...
  async getCallSummary(callId: string): Promise<CallSummaryResponse> {
    const queryParams = new URLSearchParams();
    queryParams.append('call_id', callId);
    const response = await fetch(`${API_BASE_URL}/dashboard-call-summary?${queryParams}`, {
      headers: getAuthHeaders(),
    });
    return handleResponse(response);
  },
  ...
  async getClinicalKPIs(_params?: { region?: string; start_date?: string; end_date?: string }): Promise<ClinicalKPIs> {
    // Return empty data - these advanced metrics need to be implemented
